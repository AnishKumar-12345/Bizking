...
export default {
  ...
  data(){
    return{     
      ...
      pageSize4: 10,
      headers: [
        { text: 'SA Name', value: 'sa_name' },
        { text: 'Route Name', value: 'route_name' },
        { text: 'Store Assigned', value: 'stores_count' },
        { text: 'Visited Stores', value: 'visited_stores_count' },
  ...
  computed:{
    ...
    filteredvisithealth() {
      ...
      return this.visitTrack.filter(item => {
        return (
  ...
  methods:{
    ...
    getSAvisittarck(itm){
      this.visitTrack=[]
      this.loading = true
      this.getVisitTrack(itm).then(response => {
        ...
        if(response.data.status == 1){
          ...
          this.visitTrack = response.data.data
